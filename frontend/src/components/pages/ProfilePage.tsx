...
//   try {
//     const data: ApiUser = await apiService.getProfile();
//  // ✅ type-safe
...
//     alert((error as { detail?: string }).detail || 'Failed to load profile');
//   } else {
...
//     const updatedProfile: UserProfile = await apiService.updateProfileWithImage(
//       profile.id,
//       formDataToSend
//     );
...
const ProfilePage: React.FC<ProfilePageProps> = ({ onLogout }) => {
  ...
  const fetchProfile = async () => {
    try {
      const data: UserProfile = await apiService.getProfile();
