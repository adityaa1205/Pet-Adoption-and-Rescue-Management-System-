...
const PetCard: React.FC<PetCardProps> = ({ pet }) => {
  ...
  // Safe truncation of description
  const truncateDescription = (text?: string) => {
    if (!text) return 'No description available';
    const lines = text.split('\n');
    ...
    <div className="bg-white rounded-xl shadow-lg overflow-hidden transition-transform duration-300 hover:scale-105 hover:shadow-2xl flex flex-col">
      ...
      <div className="p-5 flex flex-col flex-grow">
        ...
        <div className="space-y-2 mb-4 text-sm text-gray-700 border-t pt-4 mt-auto">
          ...
          <div className="flex items-center">
            <span>Status: <strong>{pet.is_vaccinated ? 'Vaccinated' : 'Not Vaccinated'}, {pet.is_diseased ? 'Diseased' : 'Healthy'}</strong></span>
          </div>
        </div>

        <button
          onClick={handleViewProfile}
          className="w-full px-4 py-2 font-semibold text-white bg-gradient-to-r from-orange-500 to-pink-500 rounded-lg shadow-md hover:shadow-lg mt-auto"
        >
          View Profile
        </button>
      </div>
