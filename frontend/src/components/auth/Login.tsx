...
const Login: React.FC = () => {
  ...
  const [showPassword, setShowPassword] = useState(false);
  const [errors, setErrors] = useState<Record<string, string>>({});
  const [loading, setLoading] = useState(false); // API call spinner (button)
  const [message, setMessage] = useState('');
  const [showPetLoader, setShowPetLoader] = useState(false); // GIF overlay
  ...
  setMessage('');
  setShowPetLoader(true);  // 🔹 Show GIF immediately
  ...
  try {
    ...
    // ✅ Save tokens (backend sends `access` & `refresh`)
    localStorage.setItem('access_token', loginData.access);
    localStorage.setItem('refresh_token', loginData.refresh);
    ...
    // ✅ Check superuser (matches old style, extended with localStorage)
    ...
    setShowPetLoader(false); // stop GIF if login fails
  ...
  return (
    <>
      {showPetLoader && <LoadingScreen message="Loading..."/>}
            ...
            >
              ...
              >
                ...
                <div className="bg-white/20 backdrop-blur-sm rounded-2xl p-6 border border-white/30">
                  ...
                  <p className="text-white/90 text-sm leading-relaxed">
                    Connect lost pets with loving families. Our platform helps coordinate rescue efforts,
                    track adoptions, and build a community dedicated to animal welfare.
                  </p>
                  <div className="mt-4 flex items-center justify-center text-orange-200">
                    <Heart className="w-4 h-4 mr-1" />
                    <span className="text-sm font-medium">Every pet deserves a loving home</span>
