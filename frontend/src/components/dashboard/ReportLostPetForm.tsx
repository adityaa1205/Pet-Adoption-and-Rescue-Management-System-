// src/pages/PetOwnerPage/ReportLostPetForm.tsx
...
const ReportLostPetForm: React.FC<ReportLostPetFormProps> = ({ onClose, onSuccess }) => {
    const [formData, setFormData] = useState<Omit<LostPetRequestCreate, 'pet_image'>>({
        ...
        report: { pet_status: 'Lost', report_status: 'Pending' },
        medical_history: {
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        if (!pet.name || !pet.pet_type) {
            toast.error("Pet Name and Type are required.");
            return;
        }
        if (!pet.description) {
        ...
        if (!pet.address || !pet.city || !pet.state || !pet.pincode) {
            toast.error("Please fill all 'Last Known Location' fields.");
            return;
        }
            ...
            {isSuccess ? (
                ...
                <div className="bg-white dark:bg-dark-background rounded-xl shadow-2xl p-8 w-full max-w-md text-center transform transition-all animate-fade-in">
                    ...
                    <p className="text-gray-600 mt-2 dark:text-dark-neutral">
                        Your lost pet report has been submitted for review. We hope you find your pet soon.
                    </p>
                    <button
                        onClick={onClose}
                ...
                <div className="bg-white rounded-2xl shadow-2xl w-full max-w-4xl max-h-[90vh] flex transform transition-all overflow-hidden" onClick={e => e.stopPropagation()}>
                    <div className="w-1/3 bg-gradient-to-br from-orange-50 to-orange-100 p-8 flex-col items-center justify-center hidden md:flex">
                        ...
                        <h3 className="text-3xl font-bold text-orange-900 mt-4 break-words text-center">
                            {formData.pet.name || 'Lost Pet Report'}
                        </h3>
                    ...
                    <div className="w-full md:w-2/3 flex flex-col">
                        <div className="flex items-center justify-between p-6 border-b">
                            <h3 className="text-2xl font-semibold text-gray-800">Report Lost Pet</h3>
                        ...
                        <form onSubmit={handleSubmit} className="p-6 space-y-6 overflow-y-auto">
                            <fieldset>
                                ...
                                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                                    <div><label htmlFor="name">Pet Name <span className="text-red-500">*</span></label><input type="text" name="name" id="name" value={formData.pet.name} onChange={handlePetChange} className="form-input" required /></div>
                                    <div><label htmlFor="pet_type">Pet Type <span className="text-red-500">*</span></label><input type="text" name="pet_type" id="pet_type" value={typeof formData.pet.pet_type === 'object' ? formData.pet.pet_type.type : formData.pet.pet_type || ''} onChange={handlePetChange} required className="form-input" placeholder="e.g., Dog, Cat" /></div>
                                    ...
                                    <div><label htmlFor="gender">Gender</label><select name="gender" id="gender" value={formData.pet.gender} onChange={handlePetChange} className="form-input"><option>Male</option><option>Female</option></select></div>
                                    <div><label htmlFor="age">Age (years)</label><input type="number" name="age" id="age" value={formData.pet.age || ''} onChange={handlePetChange} min="0" className="form-input" /></div>
                            ...
                            <fieldset>
                                <legend className="text-lg font-semibold text-gray-700 flex items-center mb-3"><MapPin className="w-5 h-5 mr-2 text-orange-500" /> Last Known Location <span className="text-red-500">*</span></legend>
                                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                        ...
                        <div className="flex items-center justify-end p-6 border-t mt-auto">
                            ...
                            <button type="submit" onClick={handleSubmit} disabled={submitting} className="ml-3 px-5 py-2.5 text-sm font-medium text-white bg-orange-500 rounded-lg hover:bg-orange-600 disabled:bg-orange-300">{submitting ? 'Submitting...' : 'Submit Report'}</button>
                        </div>
                    </div>
