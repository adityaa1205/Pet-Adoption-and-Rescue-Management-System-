...
import { PlusCircle, ListChecks, Pencil, Trash2 } from 'lucide-react';
...
const PetOwnerPage: React.FC = () => {
    ...
    const handleEditSuccess = () => {
        ...
        setEditingPet(null);
        fetchLostPets(); // Auto-refreshes the list
    };

    const handleDeletePet = async (pet: Pet) => {
        if (window.confirm(`Are you sure you want to permanently delete ${pet.name}? This action cannot be undone.`)) {
            try {
    ...
    return (
        <div className="container mx-auto p-6 bg-gray-50 min-h-screen">
            ...
            <div className="flex justify-between items-center mb-8 pb-4 border-b">
                <div>
                    <h1 className="text-4xl font-bold text-gray-800">Pet Owner Hub</h1>
                    <p className="text-gray-600 mt-1">Manage your requests and report lost pets.</p>
                </div>
                <div className="flex items-center space-x-4">
                    {/* This button is now handled by the main Dashboard component */}
                    {/* <button onClick={() => setIsRequestsModalOpen(true)} className="flex items-center px-4 py-2 font-semibold text-gray-700 bg-white border rounded-lg shadow-sm hover:bg-gray-100 transition-colors">
                        <ListChecks className="w-5 h-5 mr-2" /> My Requests
                    </button> */}
                    <button onClick={() => setIsReportModalOpen(true)} className="flex items-center px-4 py-2 font-semibold text-white bg-orange-500 rounded-lg shadow-md hover:bg-orange-600 transition-colors">
                        <PlusCircle className="w-5 h-5 mr-2" /> Report a Lost Pet
                    </button>
                </div>
            </div>

            <h2 className="text-2xl font-bold text-gray-700 mb-6">My Reported Lost Pets</h2>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-2 xl:grid-cols-3 gap-8">
                    {lostPets.length > 0 ? (
                        ...
                        <div className="col-span-full text-center py-12 bg-white rounded-lg shadow">
                            <h3 className="text-xl font-semibold text-gray-700">No Lost Pets Found</h3>
                            <p className="text-gray-500 mt-2">You have not reported any lost pets yet.</p>
                        </div>
                    )}
                </div>
            )}
            ...
            {/* Modals */}
            {selectedPet && <PetDetailsModal pet={selectedPet} onClose={handleCloseDetails} onPrimaryAction={handleEditFromDetails} primaryButtonLabel="Edit Pet" />}
